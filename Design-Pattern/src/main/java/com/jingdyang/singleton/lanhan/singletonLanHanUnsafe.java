 ...
 */
public class singletonLanHanUnsafe {
    private static singletonLanHanUnsafe instance;

    // 私有构造方法
    private singletonLanHanUnsafe() {
    }

    // 对外提供静态方法获取该对象
    public static singletonLanHanUnsafe getInstance() {
        if (instance == null) {
            instance = new singletonLanHanUnsafe();
        }
        return instance;
    }
}
